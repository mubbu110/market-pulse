...
if data is not None:
    try:
        ...
        cur = float(close_series.iloc[-1].item() if hasattr(close_series.iloc[-1], 'item') else close_series.iloc[-1])
        prv = float(close_series.iloc[-2].item() if hasattr(close_series.iloc[-2], 'item') else close_series.iloc[-2])
        ch = cur - prv
...
if data is not None and len(data) >= 40:
    ...
    if w is not None and r is not None:
        ...
        try:
            rr = float(r.iloc[-1].item() if hasattr(r.iloc[-1], 'item') else r.iloc[-1])
            status = "Overbought" if rr > 70 else "Oversold" if rr < 30 else "Neutral"
            c1.metric("RSI", f"{rr:.0f}", status)
            
            ww = float(w.iloc[-1].item() if hasattr(w.iloc[-1], 'item') else w.iloc[-1])
            cc = float(data['Close'].iloc[-1].item() if hasattr(data['Close'].iloc[-1], 'item') else data['Close'].iloc[-1])
