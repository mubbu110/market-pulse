...
def fetch_stock_data(symbol):
    ...
    try:
        ...
        return data if len(data) >= 220 else None
    except Exception:
        return None
...
def fetch_stock_news(symbol, company_name):
    query = f'"{company_name}" OR {symbol} stock NSE India'
    feeds = [
        f"https://news.google.com/rss/search?q={requests.utils.quote(query)}&hl=en-IN&gl=IN&ceid=IN:en",
        f"https://news.google.com/rss/search?q={requests.utils.quote(symbol + ' share price news')}&hl=en-IN&gl=IN&ceid=IN:en",
    ]
    news = []
    seen = set()
    for url in feeds:
...
with st.sidebar:
    ...
    st.markdown("---")
    selected_label = st.selectbox("Select Nifty 500 Stock", stocks["Label"].tolist(), index=0)
    selected_row = stocks.loc[stocks["Label"] == selected_label].iloc[0]
    symbol = selected_row["Symbol"]
    company_name = selected_row["Company Name"]
    st.caption("Live Nifty 500 list loaded." if live_constituents else "Fallback list loaded. Type a symbol below if needed.")

    manual_symbol = st.text_input("Override NSE Symbol", value="")
...
if tech is None:
    st.warning(f"Could not load enough price data for {symbol}. Try another NSE symbol.")
else:
