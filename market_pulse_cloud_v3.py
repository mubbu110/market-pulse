...
if "selected_headline_index" not in st.session_state:
    st.session_state.selected_headline_index = 0
if "selected_headline" not in st.session_state:
    st.session_state.selected_headline = None
if "news_message" not in st.session_state:
...
def get_news():
    ...
    feeds = [
        ...
        "https://news.google.com/rss/search?q=Nifty%2050%20stock%20market%20India&hl=en-IN&gl=IN&ceid=IN:en",
    ]
    headers = {
        ...
        try:
            ...
            for entry in feed.entries[:8]:
                ...
                if key in seen:
                    continue
                seen.add(key)
                news.append({"title": title, "source": url.split("/")[2]})
        except Exception:
            continue
    return news[:12]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### News")
    if st.button("Fetch Live News", width="stretch", key="fetch"):
        st.session_state.news = get_news()
        st.session_state.manual_headline = ""
        st.session_state.selected_headline = None
        st.session_state.selected_headline_index = 0
        st.session_state.news_message = (
            f"Loaded {len(st.session_state.news)} headlines."
            if st.session_state.news
            else "No live headlines found. Paste a headline and use Analyze Headline."
        )
    ...
    if st.button("Analyze Headline", width="stretch", key="analyze_headline"):
        st.session_state.manual_headline = st.session_state.headline.strip()
        st.session_state.selected_headline = None
        st.session_state.news = []
        st.session_state.news_message = (
            "Analyzing pasted headline."
            if st.session_state.manual_headline
            else "Paste a headline first."
        )
    if st.button("Clear", width="stretch", key="clear_sentiment"):
        st.session_state.news = []
        st.session_state.headline = ""
        st.session_state.manual_headline = ""
        st.session_state.selected_headline = None
...
if st.session_state.news:
    st.success(f"Loaded {len(st.session_state.news)} headlines. Select one headline to analyze.")
    labels = [
    ...
    selected_label = st.radio(
        "Latest Headlines",
        labels,
        index=min(st.session_state.selected_headline_index, len(labels) - 1),
        label_visibility="visible",
    )
    st.session_state.selected_headline_index = labels.index(selected_label)

    selected = st.session_state.news[st.session_state.selected_headline_index]
    if st.button("Analyze Selected News", width="stretch", key="analyze_selected_news"):
        st.session_state.selected_headline = selected

elif st.session_state.manual_headline:
    st.session_state.selected_headline = {"title": st.session_state.manual_headline, "source": "Manual"}

if st.session_state.selected_headline:
    selected_headline = st.session_state.selected_headline
    score, label, color = sentiment(selected_headline["title"])
    signal = "BUY" if score >= 4 else "SELL" if score <= -4 else "HOLD"
    signal_class = "buy" if score >= 4 else "sell" if score <= -4 else "hold"

    c1, c2 = st.columns([1.5, 2])
    with c1:
        st.markdown(
            f'<div class="card {signal_class}"><div>Impact Signal</div><div class="signal">{signal}</div><div>{score:+.1f}/10</div></div>',
            unsafe_allow_html=True,
        )
    with c2:
        title = escape(selected_headline["title"])
        source = escape(selected_headline["source"])
        st.markdown(
            f'<div class="card"><div><strong>Selected News</strong></div><div style="margin-top: 8px;">{title}</div><small style="color: #a5b4fc;">Source: {source} | {label}</small></div>',
            unsafe_allow_html=True,
        )

else:
    st.info("Fetch live news, select one headline, then click Analyze Selected News. You can also paste one headline in the sidebar.")
