...
.stApp { background: linear-gradient(135deg, #0a0e27 0%, #1a1f3a 50%, #0d1629 100%); color: #e0e7ff; }
section[data-testid="stSidebar"] { background: linear-gradient(180deg, #0f1629 0%, #1a1f3a 100%); border-right: 1px solid rgba(99, 102, 241, 0.2); }
section[data-testid="stSidebar"] * { color: #e0e7ff !important; }
.title { font-family: 'Poppins', sans-serif; font-size: 48px; font-weight: 800; background: linear-gradient(135deg, #6366f1, #a78bfa, #ec4899); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.subtitle { color: #a5b4fc; letter-spacing: 2px; text-transform: uppercase; font-size: 12px; font-weight: 600; }
...
.signal { font-family: 'Poppins', sans-serif; font-size: 42px; font-weight: 800; margin: 12px 0; }
.section { font-family: 'Poppins', sans-serif; font-size: 22px; font-weight: 700; color: #c7d2fe; margin: 24px 0 16px 0; border-bottom: 2px solid rgba(99, 102, 241, 0.3); padding-bottom: 8px; }
.metric { background: rgba(30, 27, 75, 0.4); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 12px; padding: 16px; text-align: center; }
.metric-label { font-size: 11px; color: #a5b4fc; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; margin-bottom: 8px; }
.metric-value { font-family: 'Poppins', sans-serif; font-size: 24px; font-weight: 700; color: #6366f1; }
.disclaimer { background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.4); border-radius: 12px; padding: 16px; color: #fca5a5; font-size: 12px; line-height: 1.6; }
.stButton > button { background: linear-gradient(135deg, #6366f1, #8b5cf6) !important; color: white !important; border: none !important; border-radius: 10px !important; font-weight: 700 !important; }
.stTextArea > div > div > textarea { background: rgba(30, 27, 75, 0.6) !important; color: #e0e7ff !important; border: 1px solid rgba(99, 102, 241, 0.3) !important; }
</style>
...
with st.sidebar:
    ...
    if st.button("🔄 Fetch Live News", use_container_width=True, key="fetch_news"):
        ...
        st.success(f"Got {len(st.session_state.news_list)} headlines")
    st.markdown("**OR**")
...
if d is not None:
    ...
    ch, pct = cur - prv, (cur - prv) / prv * 100
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        c = "#6ee7b7" if ch > 0 else "#fca5a5"
        a = "▲" if ch > 0 else "▼"
        st.markdown(f'<div class="metric"><div class="metric-label">Nifty50</div><div class="metric-value">{cur:,.0f}</div><div style="color: {c}; font-size: 12px; margin-top: 4px;">{a} {abs(pct):.2f}%</div></div>', unsafe_allow_html=True)
    with col2:
...
if d is not None and len(d) >= 40:
    ...
    if w is not None and r is not None:
        ...
        with col1:
            rr = r.iloc[-1]
            if rr > 70:
                st_text, c = "Overbought", "#fca5a5"
            elif rr < 30:
                st_text, c = "Oversold", "#6ee7b7"
            else:
                st_text, c = "Neutral", "#a5b4fc"
            st.markdown(f'<div class="metric"><div class="metric-label">RSI</div><div style="color: {c}; font-size: 18px; font-weight: 700;">{st_text}</div></div>', unsafe_allow_html=True)
        with col2:
            if d['Close'].iloc[-1] > w.iloc[-1]:
                st_text, c = "Above", "#6ee7b7"
            else:
                st_text, c = "Below", "#fca5a5"
            st.markdown(f'<div class="metric"><div class="metric-label">WMA</div><div style="color: {c}; font-size: 18px; font-weight: 700;">{st_text}</div></div>', unsafe_allow_html=True)
        with col3:
            if m[0] is not None and m[1] is not None and len(m[0]) > 0:
                st_text = "Bullish" if m[0].iloc[-1] > m[1].iloc[-1] else "Bearish"
                c = "#6ee7b7" if m[0].iloc[-1] > m[1].iloc[-1] else "#fca5a5"
            else:
                st_text, c = "N/A", "#a5b4fc"
            st.markdown(f'<div class="metric"><div class="metric-label">MACD</div><div style="color: {c}; font-size: 18px; font-weight: 700;">{st_text}</div></div>', unsafe_allow_html=True)
...
if h:
    ...
    with col2:
        ...
        st.markdown(f'<div class="card"><div style="font-size: 12px; color: #a5b4fc; margin-bottom: 12px;">SPLIT</div><div style="display: flex; gap: 12px;"><div style="flex: 1; text-align: center;"><div style="color: #6ee7b7; font-size: 18px; font-weight: 700;">{bl}</div><div style="font-size: 10px;">Bull</div></div><div style="flex: 1; text-align: center;"><div style="color: #fcd34d; font-size: 18px; font-weight: 700;">{ne}</div><div style="font-size: 10px;">Neutral</div></div><div style="flex: 1; text-align: center;"><div style="color: #fca5a5; font-size: 18px; font-weight: 700;">{be}</div><div style="font-size: 10px;">Bear</div></div></div></div>', unsafe_allow_html=True)
    ...
    st.info("👈 Fetch news or paste headline")
...
if st.button("Run Backtest", use_container_width=True, key="backtest"):
    with st.spinner("Testing..."):
        st.session_state.backtest_data = bt()

if st.session_state.backtest_data:
    r = st.session_state.backtest_data
    col1, col2, col3 = st.columns(3)
    with col1:
        c = "#6ee7b7" if r["return"] > 0 else "#fca5a5"
        st.markdown(f'<div class="metric"><div class="metric-label">Return</div><div class="metric-value" style="color: {c};">{r["return"]:.2f}%</div></div>', unsafe_allow_html=True)
    with col2:
